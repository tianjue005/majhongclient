...
public class UserLoginScripts : MonoBehaviour {
    ...
    public void doLogin()
    {
        ...
        CustomSocket.getInstance().sendMsg(new TestMsgRequest(null));
        
    }

    public void TestMesgCallBack(ClientResponse respone)
    {
        if (respone.status == 1)
        {
            Debug.Log("TestMesgCallBack:" + respone.message);
        }
    }
